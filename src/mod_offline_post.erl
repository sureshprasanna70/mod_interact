...
%%% This program is distributed in the hope that it will be useful,
	...
	Bthread=xml:get_path_s(Packet,[{elem,list_to_binary("thread")},cdata]),
	Bpacketid=xml:get_tag_attr_s(list_to_binary("id"),Packet),
	Type = xml:get_tag_attr_s(list_to_binary("type"), Packet),
	[_,_,_,Properties]=element(4,Packet),
	[Propertylist]=element(4,Properties),
	[_,Propertytype]=element(4,Propertylist),
	Btype=element(2,lists:nth(1,element(4,Propertytype))),
	PostUrl = "http://54.183.143.220/pushNotification.php",
	...
	"type=",Btype,Sep,
	"thread=",url_encode(binary_to_list(Bthread)),Sep,
	"packetid=",url_encode(binary_to_list(Bpacketid)),Sep],
	httpc:request(post, {PostUrl, [], "application/x-www-form-urlencoded", list_to_binary(Post)},[],[]),
	?INFO_MSG("Sending post request to ~s ", [Packet]),
	ok;
true ->
ok
end.
